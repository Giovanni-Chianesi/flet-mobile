...
def main(page: ft.Page):
    ...
    def build_task_row(t: dict, atualizar_contador=None) -> ft.Container:
    ...
    def view_lista() -> ft.View:
        ...
        contador = ft.Text(color="#A9B1D6", size=16)
        ...
        tarefas_ordenadas = sorted(
            tasks,
            key=lambda tarefa: PRIORITY_ORDER[tarefa["priority"]],
        )
        ...
        return ft.View(
            ...
            controls=[contador, lista_view],
            floating_action_button=ft.FloatingActionButton(
                icon=ft.Icons.ADD, on_click=lambda e: page.navigate("/nova"), bgcolor=BG_DESTAQUE
            ),
        )

    # Tela: Nova tarefa ou edição de tarefa
    def view_nova(tarefa: dict | None = None) -> ft.View:
        titulo = ft.TextField(
        ...
        return ft.View(
            route=f"/editar/{tarefa['id']}" if tarefa else "/nova",
            appbar=ft.AppBar(title=ft.Text("Editar tarefa" if tarefa else "Nova tarefa")),
        ...
        return ft.View(
            ...
            controls=[
                ...
                ft.ElevatedButton(
                    "Editar",
                    icon=ft.Icons.EDIT,
                    on_click=lambda e: page.navigate(f"/editar/{task_id}"),
                    bgcolor=BG_DESTAQUE,
                    color="#1B2E3D",
                ),
                ft.ElevatedButton(
    ...
    def route_change(e):
        ...
        if troute.match("/editar/:id"):
            tarefa = next((t for t in tasks if t["id"] == int(troute.id)), None)
            if tarefa:
